...
class TNTSearchEngine extends Engine
{
    ...
    private function discardIdsFromResultSetByConstraints($builder, $searchResults)
    {
        ...
        //dd($subQualifiedKeyName);

        $discardIds = $builder->model->newQuery()
            ->select($qualifiedKeyName)
            ->leftJoin(DB::raw('(' . $sub->getQuery()->toSql() .') as '.config('scout.prefix').'sub'), $subQualifiedKeyName, '=', $qualifiedKeyName)
     ...
     //   dd('$qualifiedKeyName:'.$qualifiedKeyName .',$subQualifiedKeyName'.$subQualifiedKeyName. ',$sub:'.$sub->getQuery()->toSql().',$builder->model->getKeyName():'.$builder->model->getKeyName());
